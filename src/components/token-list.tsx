...
import { Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, Avatar, Card, CardBody, Divider, Button } from "@heroui/react";
...
export const TokenList: React.FC<TokenListProps> = ({ viewMode, onSelectToken }) => {
  const [commentModalToken, setCommentModalToken] = React.useState<{id: string, symbol: string, image: string} | null>(null);
  const [repostModalToken, setRepostModalToken] = React.useState<{id: string, symbol: string, image: string} | null>(null);
  ...
  const handleComment = (token: {id: string, symbol: string, image: string}, e: React.MouseEvent) => {
    e.stopPropagation();
    setCommentModalToken(token);
  };
  
  const handleRepost = (token: {id: string, symbol: string, image: string}, e: React.MouseEvent) => {
    e.stopPropagation();
              ...
              <CardBody className="p-3">
                ...
                <div className="grid grid-cols-2 gap-1 text-xs">
                  <div>
                    ...
                    <p className="font-medium">${token.priceUsd || token.price}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-default-400">24h</p>
                    <p className={getPercentageClass(token.change24h)}>{token.change24h}%</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <p className="text-[10px] text-default-400">Liquidity</p>
                ...
                <div className="flex justify-between items-center">
                  <Button 
                    ...
                    startContent={<Icon icon="lucide:message-circle" size={14} />}
                    onPress={(e) => handleComment({
                      id: token.id,
                      symbol: token.symbol,
                      image: token.image
                    }, e)}
                  ...
                  <Button 
                    ...
                    startContent={
                      <Icon 
                        ...
                        size={14} 
                        className={likedTokens.has(token.id) ? "text-danger" : ""}
                      />
                    }
